...
from prune_weight import fine_grained_prune
...
def plot_weight_distribution(model, bins=256, count_nonzero_only=False):
    ...
    plt.savefig(os.path.join(args.output, 'weight_distribution.png'))
...
def plot_num_parameters_distribution(model):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.output, 'num_params_distribution.png'))


def plot_num_filters_distribution(model):
    ...
    plt.xticks(rotation=60)
    plt.tight_layout()
    plt.savefig(os.path.join(args.output, 'num_filters_distribution.png'))
...
def plot_weight_sensitivity_scan(model):
    ...
    dense_model_accuracy = evaluate(model, dataloader['test'], criterion, device)
    ...
    fig.tight_layout()
    fig.subplots_adjust(top=0.925)
    plt.savefig(os.path.join(args.output, 'weight_sensitivity_scan.png'))
...
if __name__ == "__main__":
    ...
    #plot_num_filters_distribution(net)
    plot_weight_sensitivity_scan(net)
