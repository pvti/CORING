...
def get_correlation_mat(all_filters_u_dict, decomposer="hosvd", criterion="VBD_dis"):
    ...
    fold = get_num_unfold(decomposer)
...
def get_rank(weight, decomposer="hosvd", criterion="VBD_dis", strategy="min_sum"):
    ...
    for i_filter in tqdm(range(num_filters)):
        ...
        u = decompose(filter, decomposer=decomposer)
        all_filters_u_dict[i_filter] = [i.tolist() for i in u]

    correl_matrix = get_correlation_mat(all_filters_u_dict, decomposer, criterion)
