...
parser.add_argument(
    '--pretrain_dir',
    type=str,
    default='checkpoint/cifar/cifar10/vgg_16_bn.pt',
...
args.job_dir = osp.join(args.job_dir, args.arch, args.decomposer, args.criterion)
if not osp.isdir(args.job_dir):
    os.makedirs(args.job_dir)

utils.record_config(args)
now = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
logger = utils.get_logger(osp.join(args.job_dir, 'measure_time_'+now+'.log'))

name = args.decomposer + '_' + args.criterion
wandb.init(
    name=name,
    project='ComplexityComparison',
    config=vars(args)
)
...
prefix_folder = osp.join(args.rank_conv_prefix, args.arch,
                         args.decomposer, args.criterion)
...
def measure_time(weight):
    """Measure time based on decomposer and criterion"""
    num_filters = weight.size(0)

    if args.decomposer == 'hosvd':
        all_filters_u_dict = {}
        for i_filter in tqdm(range(num_filters)):
            filter = weight.detach()[i_filter, :]
            u = decompose(filter, decomposer=args.decomposer)
        ...
        for i in tqdm(range(num_filters)):
            for j in range(num_filters):
                ui = all_filters_u_dict[i]
                uj = all_filters_u_dict[j]
                ui1 = torch.tensor(ui[0])
                uj1 = torch.tensor(uj[0])
                start = timer()
                for _ in range(args.seed):
                    similarity(ui1, uj1, criterion=args.criterion)
                end = timer()
                total_time += (end-start)/args.seed

    elif args.decomposer == 'full':
        total_time = 0
        for i in tqdm(range(num_filters)):
            for j in range(num_filters):
                fi = weight.detach()[i, :]
                ...
                start = timer()
                for _ in range(args.seed):
                    similarity(fi, fj, criterion=args.criterion)
                end = timer()
                total_time += (end-start)/args.seed
...
def rank_vgg(model, state_dict):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            time = measure_time(weight)
            list.append(time)
            wandb.log({'num_filters': weight.size(0), 'time': time})
